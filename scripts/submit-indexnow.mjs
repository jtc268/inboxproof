...
const keyLocation=base+'/'+key+'.txt';if((await get(keyLocation)).trim()!==key)throw Error('Root ownership file does not match');
...
if(process.argv.includes('--submit')&&changed.length){
 ...
 const response=await fetch('https://api.indexnow.org/indexnow',{method:'POST',headers:{'Content-Type':'application/json; charset=utf-8'},body:JSON.stringify({host:'inboxproof.email',key,keyLocation,urlList:changed}),signal:AbortSignal.timeout(30000)});
